...
export function TopNavbar({ userEmail, userRole, onOpenSidebar }: { userEmail: string; userRole: string; onOpenSidebar?: () => void }) {
  ...
  return (
    <header
      ...
      style={{ paddingTop: 'var(--app-safe-top)', height: 'var(--app-mobile-header-height)' }}
    >
      <button
        type="button"
      ...
      <div ref={searchRef} className="relative flex-1 min-w-0 max-w-sm">
        ...
        {searchOpen && searchQuery.length >= 2 && (
            <div className="fixed left-3 right-3 top-[calc(var(--app-mobile-header-height)+0.5rem)] sm:absolute sm:left-0 sm:right-auto sm:top-full sm:w-96 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden">
            {results.length === 0 ? (
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <div ref={notifRef} className="relative">
          ...
          {notifOpen && (
            <div className="fixed left-3 right-3 top-[calc(var(--app-mobile-header-height)+0.5rem)] sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-1 sm:w-80 bg-white border border-gray-200 rounded-xl shadow-lg z-50">
